...
tor_fw_port_mapping={'ge-0/0/1':'fw00-',
                     'ge-1/0/1':'fw00-',
                     'ge-0/0/2':'fw01-',
                     'ge-1/0/2':'fw01-'}
...
control_interfaces=['ctrl-ncm-A0/16', 'ctrl-ncm-A0/18', 'ctrl-ncm-A0/24', 'ctrl-ncm-A0/25', 'ctrl-ncm-A0/44', 'ctrl-ncm-A0/48',  'ctrl-ncm-A0/49', 'ctrl-ncm-A0/51' , 'ctrl-ncm-A0/52',
                   'ctrl-ncm-B0/16', 'ctrl-ncm-B0/18', 'ctrl-ncm-B0/24', 'ctrl-ncm-B0/25', 'ctrl-ncm-B0/44', 'ctrl-ncm-B0/48', 'ctrl-ncm-B0/49', 'ctrl-ncm-B0/51' , 'ctrl-ncm-B0/52' ]
...
expected_eth_controller_description='Mellanox Technologies MT27800 Family [ConnectX-5]'
...
def get_secret(fw_name):
    ...
    if json.loads(secret).get('GITHUB_TOKEN'):     
        ...
        print("Secret Manager does not have the information for GITHUB TOKEN \
...
class ServiceNode():
    ...
    def __create_connection(self):
        connection = manager.connect(host=self.node_ip,
                            port=830,
                            username="msozer",
    ...
    def validate_cl_version_nce_connectivity(self):
        ...
        cluster_type=re.findall(r'System Type:\sCL-\d*',output)[0].split(":")[1].strip()
        ...
        for line in output.split('\n'):
            if '|' in line and 'NC' in line:
                ...
                nce[f"{switch_type}-{switch_id}"] = {'Admin': admin, 'Operational': operational}
    ...
    def validate_cluster_type(self,expected_cluster_type:str=expected_cluster_type):
        ...
        with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log') as result_file: 
            result_file.write (time.asctime() + " " + result + '\n')
    ...
    def validate_software(self, expected_system_version:str=expected_system_version):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            system_version=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['oper-items']['system-version']
        except KeyError as key:
            result = f"RPC reply does not have the neccessary key: {key}."
        ...
        with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
            result_file.write (time.asctime() + " " + result + '\n')
    ...
    def validate_ncp_connectivity(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncp_data = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result = f"RPC reply does not have the neccessary key: {key}."        
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncf_connectivity(self):
        ...
        try:
            ncf_data = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result = f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncc_connectivity(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
          ncc_data = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncm_connectivity(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncm_data = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncm:ncms']['ncm']
        except KeyError as key:
            result = f"RPC reply does not have the neccessary key:{key}."        
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncp_fabric_connections(self):
        rpc_reply= self.connection.get(("subtree", filters.filter_ncp_fabric_connection))
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncp_data = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
        ...
        for n,ncp in enumerate(ncp_data):
            try:
                number_of_connection=ncp['oper-items']['dn-sys-backplane:backplane-statistics']['current-connected-fabric-interfaces']
            except KeyError as key:
    ...
    def validate_ncp_active_lanes(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncp_data = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sy s-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
        for n,ncp in enumerate(ncp_data):
            neighbor_connections=ncp['oper-items']['dn-sys-backplane:backplane-reachability-statistics']['neighbor-node']
            for neighbour in neighbor_connections:
                if neighbour["reachability-state"] == 'reachable':
                    if neighbour["current-reachable-active-lanes"] == "48" and "NCF" in neighbour['neighbor-node-name']:
                        result=f'NCP-{n} has {neighbour["current-reachable-active-lanes"]} active lanes towards {neighbour["neighbor-node-name"]}'
                    elif neighbour["current-reachable-active-lanes"] == "96" and "NCP" in neighbour['neighbor-node-name']:
                        result=f'NCP-{n} has {neighbour["current-reachable-active-lanes"]} active lanes towards {neighbour["neighbor-node-name"]}'
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
    ...
    def validate_control_connections(self):
        ...
        for interface in interfaces:
            rpc_reply=self.connection.get(("subtree",filters.filter_backbone_control_connections(interface=interface)))
            dicdata = xmltodict.parse(rpc_reply.xml)
            try:
                expected_nb_interface=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-interfaces:interfaces']['interface']['ncm-interfaces']['oper-items']['expected-neighbor-interface']['#text']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
            ...
            try:
                actual_nb_interface=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-interfaces:interfaces']['interface']['ncm-interfaces']['oper-items']['actual-neighbor-interface']['#text']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_mgm_ip(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            route_info=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-oob-mgmt-route:management-route']['vrfs']['vrf']['static']['ipv4']['routes']['route']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        dicdata= xmltodict.parse(rpc_reply.xml)
        sys_name=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['config-items']['name']
        ...
        for route in route_info:
            if route['prefix']=='0.0.0.0/0':
                ...
                if cf_route == next_hop:
                    result=f'Next hop configured as {next_hop} on SN which is expected'
                else:
    ...
    def validate_nce_psus(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
          nccs_platform_info=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')   
        try:
            ncps_platform_info=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."  
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')      
        try:
            ncfs_platform_info=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for p,ncc in enumerate(nccs_platform_info):
            try:
                psus=ncc["config-items"]['dn-platform:platform']['power-supply-units']['oper-items']['psu']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncp_fan(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncp_fan_data=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."     
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncp in ncp_fan_data:
            ...
            try:
                fans=ncp['config-items']['dn-platform:platform']['fans']['oper-items']['fan']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncc_fan(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncc_fan_data=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncc in ncc_fan_data:
            try:
                fans=ncc['config-items']['dn-platform:platform']['fans']['oper-items']['fan']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncf_fan(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncf_fan_data=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncf in ncf_fan_data:
            ...
            try:
                fans=ncf['config-items']['dn-platform:platform']['fans']['oper-items']['fan']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncm_fan(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncm_fan_data=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncm:ncms']['ncm']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncm in ncm_fan_data:
            try:
                fans=ncm['config-items']['dn-platform:platform']['fans']['oper-items']['fan']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."  
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncp_temps(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncp_temps = dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."        

        for ncp in ncp_temps:

            for status in ncp["config-items"]["dn-platform:platform"]['temperature-sensors']['oper-items']['temperature-sensor']:
                if status['status'] == 'OK':
    ...
    def validate_cplds(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncps_cpld=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
        try:
            ncfs_cpld=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError:
            result=f"RPC reply does not have the neccessary key: {key}."      
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')  
        try:
            ncms_cpld=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncm:ncms']['ncm']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncp in ncps_cpld:
            for cpu in ncp["config-items"]["dn-platform:platform"]['oper-items']['cpld-version']:
        ...
        for ncf in ncfs_cpld:
            ...
            for cpu in ncf["config-items"]["dn-platform:platform"]['oper-items']['cpld-version']:
                if cpu['cpld-id']== "CPU CPLD":
                    if cpu['version']=='0.30':
        ...
        for ncm in ncms_cpld:
            ...
            try:
                elements=ncm["config-items"]["dn-platform:platform"]['oper-items']['cpld-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_onie_version(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncps_onie=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
        try:
            ncfs_onie=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."      
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')  
        try:
            ncms_onie=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncm:ncms']['ncm']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
        try:
            nccs_onie=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key: {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncp in ncps_onie:
            try:
                onie_version=ncp["config-items"]["dn-platform:platform"]['oper-items']['onie-version']
            except KeyError as key: 
                result= f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncf in ncfs_onie:
            try:
                onie_version=ncf["config-items"]["dn-platform:platform"]['oper-items']['onie-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."  
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncm in ncms_onie:
            ...
            try:
                onie_version=ncm["config-items"]["dn-platform:platform"]['oper-items']['onie-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."        
            if onie_version == expected_onie_version:
        ...
        for ncc in nccs_onie:
            try:
                onie_version=ncc["config-items"]["dn-platform:platform"]['oper-items']['onie-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key: {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_bios_version(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncps_bios=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')       
        try:
            ncfs_bios=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')       
        try:        
            ncms_bios=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncm:ncms']['ncm']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}."   
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')     
        try:        
            nccs_bios=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result= f"RPC reply does not have the neccessary key : {key}."    
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncp in ncps_bios:
            try:
                bios_version=ncp["config-items"]["dn-platform:platform"]['oper-items']['bios-version']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}." 
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')              
            try:
                bios_mode=ncp["config-items"]["dn-platform:platform"]['oper-items']['bios-mode']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')             
            if bios_version ==expected_ncp_ncf_bios_version and bios_mode==expected_bios_mode:
                result=f'BIOS version of NCP-{ncp["ncp-id"]} is {bios_version} and BIOS mode is {bios_mode} which are expected values'
            elif bios_version ==expected_ncp_ncf_bios_version and not bios_mode==expected_bios_mode:
                result=f'BIOS version of NCP-{ncp["ncp-id"]} is {bios_version} and but BIOS mode is {bios_mode}. Please check bios mode'
            elif not bios_version ==expected_ncp_ncf_bios_version and bios_mode==expected_bios_mode:
        ...
        for ncf in ncfs_bios:
            try:
                bios_version=ncf["config-items"]["dn-platform:platform"]['oper-items']['bios-version']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}." 
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')            
            try:
                bios_mode=ncf["config-items"]["dn-platform:platform"]['oper-items']['bios-mode']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncm in ncms_bios:
            try:
                bios_version=ncm["config-items"]["dn-platform:platform"]['oper-items']['bios-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')  
            try:
                bios_mode=ncm["config-items"]["dn-platform:platform"]['oper-items']['bios-mode']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."   
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')    
            if bios_version ==expected_ncm_bios_version and bios_mode==expected_bios_mode:
                result=f'BIOS version of NCM-{ncm["ncm-id"]} is {bios_version} and BIOS mode is {bios_mode} which are expected values'
            elif bios_version ==expected_ncm_bios_version and not bios_mode==expected_bios_mode:
                result=f'BIOS version of NCM-{ncm["ncm-id"]} is {bios_version} and but BIOS mode is {bios_mode}. Please check bios mode'
            elif not bios_version ==expected_ncm_bios_version and bios_mode==expected_bios_mode:
        ...
        for ncc in nccs_bios:
            try:
                bios_version=ncc["config-items"]["dn-platform:platform"]['oper-items']['bios-version']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')
            try:  
                bios_mode=ncc["config-items"]["dn-platform:platform"]['oper-items']['bios-mode']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}."    
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                    result_file.write (time.asctime() + " " + result + '\n')
            if bios_version ==expected_ncc_bios_version and bios_mode==expected_bios_mode:
                result=f'BIOS version of NCC-{ncc["ncc-id"]} is {bios_version} and BIOS mode is {bios_mode} which are expected values'
            elif bios_version ==expected_ncc_bios_version and not bios_mode==expected_bios_mode:
                result=f'BIOS version of NCC-{ncc["ncc-id"]} is {bios_version} and but BIOS mode is {bios_mode}. Please check bios mode'
            elif not bios_version ==expected_ncc_bios_version and bios_mode==expected_bios_mode:
    ...
    def validate_base_os(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncps_base_os=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
        try:          
            ncfs_base_os=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result= f"RPC reply does not have the neccessary key : {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
                result_file.write (time.asctime() + " " + result + '\n')
        try:  
            nccs_base_os=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result= f"RPC reply does not have the neccessary key : {key}."  
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncp in ncps_base_os:
            try:
                baseos_version=ncp["config-items"]["dn-platform:platform"]['oper-items']['host-os']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncf in ncfs_base_os:
            try:
                baseos_version=ncf["config-items"]["dn-platform:platform"]['oper-items']['host-os']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}."  
            if baseos_version == expected_baseos_version:
        ...
        for ncc in nccs_base_os:
            try:
                baseos_version=ncc["config-items"]["dn-platform:platform"]['oper-items']['host-os']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."              
            if baseos_version == expected_baseos_version:
    ...
    def validate_hw_revision(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)
        try:
            ncps_hw_version=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncp:ncps']['ncp']
        except KeyError as key:
            result= f"RPC reply does not have the neccessary key : {key}."  
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        try:        
            ncfs_hw_version=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncf:ncfs']['ncf']
        except KeyError as key:
            result= f"RPC reply does not have the neccessary key : {key}."
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncp in ncps_hw_version:
            try:
                hw_revision=ncp["config-items"]["dn-platform:platform"]['oper-items']['hardware-revision']
            except KeyError as key:
                result= f"RPC reply does not have the neccessary key : {key}."   
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncf in ncfs_hw_version:
            try:
                hw_revision=ncf["config-items"]["dn-platform:platform"]['oper-items']['hardware-revision']
            except KeyError as key: 
                result= f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_ncm_ncc_ipmi(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)        
        try:
            ncm_ipmi=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncm:ncms']['ncm']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}."  
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        try:        
            ncc_ipmi=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}." 
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncm in ncm_ipmi:
            try:
                ipmi_version=ncm["config-items"]["dn-platform:platform"]['oper-items']['ipmi-version']['image-1-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncc in ncc_ipmi:
            try:
                ipmi_version=ncc["config-items"]["dn-platform:platform"]['oper-items']['ipmi-version']['image-1-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
    ...
    def validate_eth_controller(self):
        ...
        dicdata = xmltodict.parse(rpc_reply.xml)        
        try:
            ncc_eth=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-system:system']['dn-sys-ncc:nccs']['ncc']
        except KeyError as key:
            result=f"RPC reply does not have the neccessary key : {key}."          
            print(result)
            with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
        ...
        for ncc in ncc_eth:
            ...
            try:
                nics=ncc["config-items"]["dn-platform:platform"]['oper-items']['on-board-management-ethernet-controller-version']
            except KeyError as key:
                result=f"RPC reply does not have the neccessary key : {key}."
                print(result)
                with open(f'/Users/muhammedsozer/Documents/AcceptanceAutomation/results/results-{self.node_ip}-{self.name}.log', 'a') as result_file: 
            ...
            for nic in nics:
                ...
                version=nic['version']
                if description ==expected_eth_controller_description and version==expected_eth_controller_version:
                    result=f'Ethernet controller of NCC-{ncc["ncc-id"]} is {description} and version is {version} which are expected'
                elif description ==expected_eth_controller_description and not version==expected_eth_controller_version:
    ...
    def validate_number_of_vADI_routes(self):
        rpc_reply=self.connection.get(('subtree', filters.filter_vrf_route_summary_vADI))
        # return rpc_reply
        dicdata = xmltodict.parse(rpc_reply.xml)
        result={}
        try:
            vrf_route_v4=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-network-services:network-services']['dn-vrf:vrfs']['vrf']['oper-items']['vrf-route-summary']['ipv4-unicast-routes']['ebgp-routes']
            vrf_route_v6=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-network-services:network-services']['dn-vrf:vrfs']['vrf']['oper-items']['vrf-route-summary']['ipv6-unicast-routes']['ebgp-routes']
        except KeyError as key:
            return f"RPC reply does not have the neccessary key : {key}."             
        if int(vrf_route_v4) < v4_global_prefix_threshold:
            result['ipv4']={}
            result['ipv4']['status']='FAILURE: '
            result['ipv4']['message']=f"Number of the IPV4 prefixes is less than expected value. Number of Prefixes: {vrf_route_v4}, Global_prefixes_threshold:{v4_global_prefix_threshold}"
        else:
            result['ipv4']={}
            result['ipv4']['status']='SUCCESS: '
            result['ipv4']['message']=f"Number of the IPV4 prefixes is above the expected value. Number of Prefixes: {vrf_route_v4}, Global_prefixes_threshold:{v4_global_prefix_threshold}"
        if int(vrf_route_v6) < v6_global_prefix_threshold:
            result['ipv6']={}
            result['ipv6']['status']='FAILURE: '
            result['ipv6']['message']=f"Number of the IPV6 prefixes is less than expected value. Number of Prefixes: {vrf_route_v6}, Global_prefixes_threshold:{v6_global_prefix_threshold}"
        else:
            result['ipv6']={}
            result['ipv6']['status']='SUCCESS: '
            result['ipv6']['message']=f"Number of the IPV6 prefixes is above the expected value. Number of Prefixes: {vrf_route_v6}, Global_prefixes_threshold:{v6_global_prefix_threshold}"
    ...
    def validate_vadi_bgp_nbrs(self): 
        rpc_reply=self.connection.get(('subtree', filters.filter_number_of_bgp_nb_vADI))
        dicdata = xmltodict.parse(rpc_reply.xml)
        result={}
        try:
            nbrs=dicdata['nc:rpc-reply']['data']['dn-top:drivenets-top']['dn-network-services:network-services']['dn-vrf:vrfs']['vrf']['protocols']['dn-bgp:bgp']['global']['global-oper']['bgp-summary']['global']['address-families']['address-family']
        except KeyError as key:
            return f"RPC reply does not have the neccessary key : {key}."  
        for nbr in nbrs:
            if nbr['address-family-type']=='ipv4-unicast' and int(nbr['number-established-neighbors']) >=number_of_expected_v4_bgp_nbr:
                result["v4"]={}
                result["v4"]['status']='SUCCESS: '
                result["v4"]['message']=f'Number of the v4 bgp nbrs meets the expectation. Number of BGP neigbors:{nbr["number-established-neighbors"]}'
                break
            else:
                result["v4"]={}
                result["v4"]['status']='FAILURE: '
                result["v4"]['message']=f'Number of the v4 bgp nbrs does not meet the expectation. Number of BGP neigbors:{nbr["number-established-neighbors"]}'                

        for nbr in nbrs:
            if nbr['address-family-type']=='ipv6-unicast' and int(nbr['number-established-neighbors']) >=number_of_expected_v6_bgp_nbr:
                result["v6"]={}
                result["v6"]['status']='SUCCESS: '
                result["v6"]['message']=f'Number of the v6 bgp nbrs meets the expectation. Number of BGP neigbors:{nbr["number-established-neighbors"]}'
                break
            else:
                result["v6"]={}
                result["v6"]['status']='FAILURE: '
                result["v6"]['message']=f'Number of the v6 bgp nbrs does not meet the expectation. Number of BGP neigbors:{nbr["number-established-neighbors"]}'    
    ...
    def validate_bundle_interface(self):
        output=self.ssh_connection.send_command("show interface bundle-60002 | tail 1")
        result={}
        ...
        if "ge100-0/0/2" in interface:
            if len(interface)==1:
                if 'up' in output:
                    result["status"]='SUCCESS: '
                    result['message']="ge100-0/0/2 is the member of Bundle-60002 and state is up"
                else:
                    result["status"]='FAILURE: '
                    result['message']="ge100-0/0/2 is the member of Bundle-60002 but state is DOWN"
            else:
                result["status"]='FAILURE: '
                result['message']="ge100-0/0/2 is the member of Bundle-60002 and but there is an unexpected member in bundle. Please check bundle configuration"
        else:
            result["status"]='FAILURE: '
...
class ToR():
    ...
    def validate_virtaul_chasis(self,table):
        ...
        number_of_members=len(structured_output['rpc-reply']['virtual-chassis-information']['member-list']['member'])
        number_of_vcp_links=0
        result={}
        members=structured_output['rpc-reply']['virtual-chassis-information']['member-list']['member']
...
class FireWall():
    ...
    def validate_tunnels(self,table):
        ...
        missing_tunnel= [item for item in expected_tunnel_names if item not in tunnel_names]
    ...
    def validate_ha_peers(self,table):
        ...
        if not len(response) == 2:
            result["status"]="FAILURE: "
            result["message"]=f"HA group does not have 2 firewalls. Please check the FW configurations"
    ...
    def validate_ha_health(self,table):
        ...
        for n,line in enumerate(lines):
            ...
            if "Configuration Status" in line:
                check[lines[n+1].split(':')[0].strip()]=lines[n+1].split(':')[1].strip()
                check[lines[n+2].split(':')[0].strip()]=lines[n+1].split(':')[1].strip()
        for line in check.values():
            if line in ['OK','in-sync'] and check["HA Health Status"] == 'OK':
                result["status"]='SUCCESS: '
                result["message"]="Firewall HA Health status is OK and config is in-sync"
            elif  check["HA Health Status"] == 'OK':
                result['status']='FAILURE: '
                result["message"]="Firewall HA Health status is OK but config is NOT in-sync."
            else:
                result['status']='SUCCESS: '
    ...
    def validate_fw_policy(self,table):
        ...
        return_elmt = requests.get('https://raw.githubusercontent.com/drivenets/NaaS-Ops/main/SITE_FINAL_V3/MF00_V3.J2', headers=headers)
        ...
        policy_config_raw=config.splitlines()[index_policy:index_policy+index_end+1]
            ...
            for sot_elmt in clean_dic:
                if not sot_elmt['policyid'] == response_elmt['policyid']:
                    ...
                    for k in list(sot_elmt.keys()): 
                        ...
                        result[response_elmt['policyid']]["message"]=f'Policy {response_elmt["policyid"]} element {k.upper()} {response_elmt[k]} does not have the right value. it should be {sot_elmt[k]}'
                        h+=1
            if h==0 and not p==len(clean_dic) :
                result[response_elmt['policyid']]={}
                result[response_elmt['policyid']]["status"]='SUCCESS: '
                result[response_elmt['policyid']]["message"]=f"Policy {response_elmt['policyid']} has the right configuration"
            if p==len(clean_dic) :
                result[response_elmt['policyid']]={}
                result[response_elmt['policyid']]["status"]='FAILURE: '
                result[response_elmt['policyid']]["message"]= f"Policy {response_elmt['policyid']} should not present"

        expected_policy=[item["policyid"] for item in clean_dic]
        configured_policy=[item["policyid"] for item in response]
                

        missing_policy= [item for item in expected_policy if item not in configured_policy]
...
class TerminalServer():
    ...
    def validate_port_mapping(self,table,site_id,terminal_server):
        ...
        expected_ts_ports_sn01=[f'1. fw02.{site_id}', '10. ncp1.sn01', f'11. pdu02.{site_id}', f'12. pdu03.{site_id}', f'2. fw03.{site_id}', f'3. tor02.{site_id}', f'4. tor03.{site_id}', f'5. ncma0.sn01', f'6. ncmb0.sn01', f'7. ncf0.sn01', f'8. ncf1.sn01', f'9. ncp0.sn01']
        expected_ts_ports_sn00=[f'1. fw00.{site_id}', '10. ncp1.sn00', f'11. pdu00.{site_id}', f'12. pdu01.{site_id}', f'2. fw01.{site_id}', f'3. tor00.{site_id}', f'4. tor01.{site_id}', f'5. ncma0.sn00', f'6. ncmb0.sn00', f'7. ncf0.sn00', f'8. ncf1.sn00', f'9. ncp0.sn00']
        elements=re.findall(r'\d+\.\s*\w+?\d*?\.\w+', output,flags=re.M)
        elements=list(set(elements))
        elements.sort()
        port_match={"00":expected_ts_ports_sn00,
                    "01":expected_ts_ports_sn01}
        if port_match[service_node_id]==elements:
            result["status"]='SUCCESS: '
            result["message"]="Terminal server port mappings and descriptions are as expected"
            results.append(result)
        else:
            missing_port_map=[item for item in port_match[service_node_id] if item not in elements]
            for port in missing_port_map:
                result['status']="FAILURE: "
                result['message']=f"{port} is missing or mapping is not to the right port in terminal server"
...
class Pdu():
    ...
    def validate_outlet_mapping(self,site_id,name,table):
        ...
        expected_match_pdu00_sn00=[f'psu.mc00.{site_id}', 'Free_Outlet_2',f'psu0.tor00.{site_id}',f'psu1.tor00.{site_id}',f'psu0.ncma0.sn00.{site_id}',
                         f'psu0.ncp0.sn00.{site_id}',f'psu0.ncf1.sn00.{site_id}',f'psu0.ncf0.sn00.{site_id}',f'psu.fw00.{site_id}','Free_Outlet_10',
                         'Free_Outlet_11',f'psu0.ncmb0.sn00.{site_id}',f'psu0.ncp1.sn00.{site_id}',f'psu1.ncc1.sn00.{site_id}',
                         f'psu1.ncc0.sn00.{site_id}', f'psu1.ncs0.sn00.{site_id}']
        
        expected_match_pdu01_sn00=[f'psu.mc01.{site_id}',f'psu.ts00.{site_id}',f'psu0.tor01.{site_id}',f'psu1.tor01.{site_id}',f'psu1.ncma0.sn00.{site_id}',
                         f'psu1.ncp0.sn00.{site_id}',f'psu1.ncf1.sn00.{site_id}',f'psu1.ncf0.sn00.{site_id}',f'psu.fw01.{site_id}','Free_Outlet_10',
                         'Free_Outlet_11',f'psu1.ncmb0.sn00.{site_id}',f'psu1.ncp1.sn00.{site_id}',f'psu2.ncc1.sn00.{site_id}',
                         f'psu2.ncc1.sn00.{site_id}', f'psu2.ncs0.sn00.{site_id}']


        expected_match_pdu02_sn01=[f'psu.mc00.{site_id}', 'Free_Outlet_2',f'psu0.tor02.{site_id}',f'psu1.tor02.{site_id}',f'psu0.ncma0.sn01.{site_id}',
                         f'psu0.ncp0.sn01.{site_id}',f'psu0.ncf1.sn01.{site_id}',f'psu0.ncf0.sn01.{site_id}',f'psu.fw02.{site_id}','Free_Outlet_10',
                         'Free_Outlet_11',f'psu0.ncmb0.sn01.{site_id}',f'psu0.ncp1.sn01.{site_id}',f'psu1.ncc1.sn01.{site_id}',
                         f'psu1.ncc0.sn01.{site_id}', f'psu1.ncs0.sn01.{site_id}']
        
        expected_match_pdu03_sn01=[f'psu.mc03.{site_id}',f'psu.ts01.{site_id}',f'psu0.tor03.{site_id}',f'psu1.tor03.{site_id}',f'psu1.ncma0.sn01.{site_id}',
                         f'psu1.ncp0.sn01.{site_id}',f'psu1.ncf1.sn01.{site_id}',f'psu1.ncf0.sn01.{site_id}',f'psu.fw03.{site_id}','Free_Outlet_10',
                         'Free_Outlet_11',f'psu1.ncmb0.sn01.{site_id}',f'psu1.ncp1.sn01.{site_id}',f'psu2.ncc1.sn01.{site_id}',
                         f'psu2.ncc1.sn01.{site_id}', f'psu2.ncs0.sn01.{site_id}']
        
        
        compare={"pdu00-sn00":expected_match_pdu00_sn00, "pdu01-sn00":expected_match_pdu01_sn00, "pdu02-sn01": expected_match_pdu02_sn01, "pdu03-sn01": expected_match_pdu03_sn01}

        remote_conn_pre = paramiko.SSHClient()
        remote_conn_pre.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        remote_conn_pre.connect(f"{self.pdu_ip}",username=f'{username}',password=f"{password}",allow_agent=False,look_for_keys=False,disabled_algorithms=dict(pubkeys=['rsa-sha2-512', 'rsa-sha2-256'], keys=['rsa-sha2-512', 'rsa-sha2-256']))
...
def upload_results_to_S3(filename, bucket, object_name=None):
    ...
    try:
        ...
        print(f"{filename} was uploaded to S3. You reach the file from https://s3.console.aws.amazon.com/s3/buckets/msozer-log-bucket?region=us-east-1&tab=objects" )
    except ClientError as e:
        logging.error(e)
        return False
    return True
